...
static int WriteHashTable(FILE* f, IndexFileOffset_t offset, HashTable* ht,
  ...
  for (int i = 0; i < ht->num_buckets; i++) {
    ...
    int32_t bucket_size  = LinkedList_NumElements(bucket);

    // write record
    int record_bytes = WriteHTBucketRecord(f,
      record_pos,
      bucket_size,
      bucket_pos);
    ...
    // write contents (if bucket isn't empty)
    if (bucket_size > 0) {
      int bucket_bytes = WriteHTBucket(f,
        bucket_pos,
        bucket,
        fn);
...
static int WriteHTBucket(FILE* f, IndexFileOffset_t offset, LinkedList* li,
  ...
  for (int i = 0; i < num_elts; i++) {
    ...
    int element_bytes = fn(f,
      element_pos,
      reinterpret_cast<HTKeyValue_t *>(result));
