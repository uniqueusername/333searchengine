  ...
  while (*cur_ptr != '\0') {
    if (isalpha(*cur_ptr)) {
      ...
      *cur_ptr = tolower(*cur_ptr);
      ...
      // so set string boundary
      *cur_ptr = '\0';

      // check if this is a word and not
      // repeated boundary characters
...
static void AddWordPosition(HashTable* tab, char* word,
  ...
  if (HashTable_Find(tab, hash_key, &kv)) {
    ...
    // fill struct
    wp->word = (char *) malloc((strlen(word) + 1) * sizeof(char));
