...
static HttpResponse ProcessFileRequest(const string& uri,
  ...
  if (read.ReadFile(&s)) {
    ...
    if (suffix == ".html" || suffix == ".htm")
      ...
      ret.set_content_type("text/jpeg");
    else if (suffix == ".png")
...
static HttpResponse ProcessQueryRequest(const string& uri,
  ...
  if (uri.find("query?terms=") != string::npos) {
    ...
    vector<hw3::QueryProcessor::QueryResult> res = qp.ProcessQuery(terms);
    ...
    if (res.size() == 0) {
      ...
      for (uint16_t i = 0; i < res.size(); i++) {
        ...
        ret.AppendToBody("\">");
        ret.AppendToBody(EscapeHtml(res[i].document_name));
        ret.AppendToBody("</a> [");

        ret.AppendToBody(std::to_string(res[i].rank));
        ret.AppendToBody("]<br>\r\n");
      }
