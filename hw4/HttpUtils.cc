...
bool IsPathSafe(const string& root_dir, const string& test_file) {
  ...
  // absolute both paths
  if (!realpath(root_dir.c_str(), dir) || !realpath(test_file.c_str(), file))
    return false;

  // check that file is a prefix of dir
  if (strlen(file) <= strlen(dir)) return false;
  if (strncmp(dir, file, strlen(dir)) != 0) return false;
  if (file[strlen(dir)] != '/') return false;

  // if none of previous statements returned, paths are safe
  return true;
}
